...
void fill_mission_data(comms_ariel::Mission* msg, uint64_t mission_id, double* waypoints,
                       uint64_t usv_reference_time, double* usv_waypoints)
{
  msg->set_mission_id(mission_id);
  for(int i=0; i<3; i++) msg->set_waypoints(i, waypoints[i]);
  msg->set_usv_reference_time(usv_reference_time);
  for(int i=0; i<3; i++) msg->set_usv_waypoints(i, usv_waypoints[i]);
}
                   ...
                   comms_ariel::USVTelemetry::States state)
{
  msg->set_latitude(latitude);
  msg->set_longitude(longitude);
  msg->set_orientation(0, orientation);
